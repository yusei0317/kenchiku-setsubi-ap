...
def main():
    ...
    with st.container(border=True):
        ...
        if not df_history.empty and 'timestamp' in df_history.columns:
            today_history = df_history[df_history['timestamp'].astype(str).str.contains(today_str)]
        else:
            today_history = pd.DataFrame()
        ...
        if st.button("▶ 苦手分野集中で学習をスタート", type="primary", use_container_width=True):
            st.toast("クイズメニューから学習を開始してください。")
